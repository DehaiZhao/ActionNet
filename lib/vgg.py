...
def convert(checkpoint_path):
  ...
  npy_param['conv1_1'] = [reader.get_tensor(net_name + '/conv1/conv1_1/weights'), reader.get_tensor(net_name + '/conv1/conv1_1/biases')]
  npy_param['conv1_2'] = [reader.get_tensor(net_name + '/conv1/conv1_2/weights'), reader.get_tensor(net_name + '/conv1/conv1_2/biases')]

  npy_param['conv2_1'] = [reader.get_tensor(net_name + '/conv2/conv2_1/weights'), reader.get_tensor(net_name + '/conv2/conv2_1/biases')]
  npy_param['conv2_2'] = [reader.get_tensor(net_name + '/conv2/conv2_2/weights'), reader.get_tensor(net_name + '/conv2/conv2_2/biases')]

  npy_param['conv3_1'] = [reader.get_tensor(net_name + '/conv3/conv3_1/weights'), reader.get_tensor(net_name + '/conv3/conv3_1/biases')]
  npy_param['conv3_2'] = [reader.get_tensor(net_name + '/conv3/conv3_2/weights'), reader.get_tensor(net_name + '/conv3/conv3_2/biases')]
  npy_param['conv3_3'] = [reader.get_tensor(net_name + '/conv3/conv3_3/weights'), reader.get_tensor(net_name + '/conv3/conv3_3/biases')]
  npy_param['conv3_4'] = [reader.get_tensor(net_name + '/conv3/conv3_4/weights'), reader.get_tensor(net_name + '/conv3/conv3_4/biases')]

  npy_param['conv4_1'] = [reader.get_tensor(net_name + '/conv4/conv4_1/weights'), reader.get_tensor(net_name + '/conv4/conv4_1/biases')]
  npy_param['conv4_2'] = [reader.get_tensor(net_name + '/conv4/conv4_2/weights'), reader.get_tensor(net_name + '/conv4/conv4_2/biases')]
  npy_param['conv4_3'] = [reader.get_tensor(net_name + '/conv4/conv4_3/weights'), reader.get_tensor(net_name + '/conv4/conv4_3/biases')]
  npy_param['conv4_4'] = [reader.get_tensor(net_name + '/conv4/conv4_4/weights'), reader.get_tensor(net_name + '/conv4/conv4_4/biases')]

  npy_param['conv5_1'] = [reader.get_tensor(net_name + '/conv5/conv5_1/weights'), reader.get_tensor(net_name + '/conv5/conv5_1/biases')]
  npy_param['conv5_2'] = [reader.get_tensor(net_name + '/conv5/conv5_2/weights'), reader.get_tensor(net_name + '/conv5/conv5_2/biases')]
  npy_param['conv5_3'] = [reader.get_tensor(net_name + '/conv5/conv5_3/weights'), reader.get_tensor(net_name + '/conv5/conv5_3/biases')]
  npy_param['conv5_4'] = [reader.get_tensor(net_name + '/conv5/conv5_4/weights'), reader.get_tensor(net_name + '/conv5/conv5_4/biases')]

  npy_param['fc6'] = [reader.get_tensor(net_name + '/fc6/weights'), reader.get_tensor(net_name + '/fc6/biases')]
  npy_param['fc7'] = [reader.get_tensor(net_name + '/fc7/weights'), reader.get_tensor(net_name + '/fc7/biases')]
  npy_param['fc8'] = [reader.get_tensor(net_name + '/fc8/weights'), reader.get_tensor(net_name + '/fc8/biases')]
...
class BaseVGG(BaseModel):
    ...
    def _create_input(self):
        ...
        self.image_a = tf.placeholder(
            tf.float32, name='image_a',
            shape=[None, self.im_height, self.im_width, self.num_channels])
        self.image_b = tf.placeholder(
            tf.float32, name='image_b',
            shape=[None, self.im_height, self.im_width, self.num_channels])

        self.label = tf.placeholder(tf.int64, [None], 'label')

        self.set_model_input([self.image_a, self.keep_prob])
        self.set_model_input([self.image_b, self.keep_prob])
        self.set_dropout(self.keep_prob, keep_prob=0.5)
        self.set_train_placeholder([self.image_a, self.label])
        self.set_train_placeholder([self.image_b, self.label])
        self.set_prediction_placeholder(self.image_a)
        self.set_prediction_placeholder(self.image_b)


class VGG19(BaseVGG):

    def _create_conv(self, input_im_a, input_im_b, data_dict):

        concat_inputs = tf.concat([input_im_a, input_im_b], axis=3)
        arg_scope = tf.contrib.framework.arg_scope
        with arg_scope([conv], nl=tf.nn.relu,
                       trainable=True, data_dict=data_dict):
            conv1_1 = conv(concat_inputs, 3, 64, 'conv1_1')
    ...
    def _create_model(self):
        ...
        with tf.name_scope('input'):
            input_im_a = self.model_input[0]
            input_im_b = self.model_input[1]
            keep_prob = self.model_input[2]

            input_im_a = tf.reshape(input_im_a, [-1, 224, 224, 3])
            input_im_b = tf.reshape(input_im_b, [-1, 224, 224, 3])

            self.layer['input_a'] = input_im_a
            self.layer['input_b'] = input_im_b
            # Convert RGB image to BGR image
            red, green, blue = tf.split(axis=3,
                                        num_or_size_splits=3,
                                        value=input_im_a)

            input_bgr_a = tf.concat(axis=3, values=[
                blue - VGG_MEAN[0],
                green - VGG_MEAN[1],
                red - VGG_MEAN[2],
            ])
            ...
            input_bgr_b = tf.concat(axis=3, values=[
                blue - VGG_MEAN[0],
                green - VGG_MEAN[1],
                red - VGG_MEAN[2],
            ])
        ...
        data_dict = {}
        if self._is_load:
            data_dict = convert(self._pre_train_path)

        conv_output = self._create_conv(input_bgr_a, input_bgr_b, data_dict)
...
class VGG19_FCN(VGG19):
    ...
    def _create_model(self):
        ...
        with tf.name_scope('input'):
            ...
            if self._is_rescale:
                input_im_a =\
                    resize_tensor_image_with_smallest_side(input_im_a, 224)
                input_im_b =\
                    resize_tensor_image_with_smallest_side(input_im_b, 224)
        ...
        conv_outptu = self._create_conv(input_bgr_a, input_bgr_b, data_dict)
