...
class BaseGradCAM(object):
    ...
    def create_model(self, input_a, input_b):
        self._create_model(input_a, input_b)

    def _create_model(self, input_a, input_b):
        pass
    ...
    def get_visualization(self, class_id=None):
        ...
        with tf.name_scope('grad_cam'):
            ...
            o_h = tf.shape(self.input_im_a)[1]
            o_w = tf.shape(self.input_im_a)[2]
...
class ClassifyGradCAM(BaseGradCAM):
    def _create_model(self, input_a, input_b):
        keep_prob = 1
        self._vis_model.create_model([input_a, input_b, keep_prob])
